...
void mctl_set_timing_params(const struct dram_para *para)
{
	...
	u16 trfc	= ns_to_t(180);  // as in reversed function
	...
	u8 trd2wr	= ((ns_to_t(4) + 17) - (ns_to_t(1) & 0xff)) & 0xff;
    trtp = 4;
	u8 tcl		= 10;
	u8 t_rdata_en	= 17;
	...
	writel(0x00020208, &mctl_ctl->dramtmg[9]);  // 8 | (2 << 8) | 0x20000
    ...
    clrsetbits_le32(&mctl_ctl->init[2], 0x0000ff0f, 0x000000d05);
	writel(0, &mctl_ctl->dfimisc);
    // mr1=0x34, mr2=0x1b, mr3 from config

	writel(0x0034001b, &mctl_ctl->init[3]); // mr2 | (mr1 << 16)
    ...
    if ((para->tpr13 & 0x20) == 0) {
        pre_dftimg0 = (tphy_wrlat - 1) | 0x2000000 | ((t_rdata_en - 1) << 16);
    } else {
        pre_dftimg0 = tphy_wrlat | 0x2000000 | (t_rdata_en << 16);
    }

	writel(pre_dftimg0 | 0x808000, &mctl_ctl->dfitmg0);
